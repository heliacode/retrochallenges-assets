-- Mega Man 2 — Wood Man Boss Fight (Buster Only)
-- Loaded loadout: full HP (28), buster equipped, no subweapon. Defeat
-- Wood Man as fast as possible. Die once and the run is over.
--
-- Mirrors boss-metalman / boss-heatman / crashman-buster-only — same
-- freeze trick, same time-grade thresholds, same death detector.
--
...
-- ---------------------------------------------------------------------------
-- Freeze trick: write 0x04 to game_mode → entity_update_dispatch short-
-- circuits, so AI / physics / projectiles / animations all halt while
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/boss-woodman.state",

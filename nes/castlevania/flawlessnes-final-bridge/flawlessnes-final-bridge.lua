-- FlawlessNES — Castlevania Final Bridge (reach Stage 17), No Damage
-- =================================================================
-- Cross the final bridge into Stage 17 without taking a hit. FlawlessNES
-- rules: you don't FAIL on a hit; every hit chips your score per the fixed
-- table (identical for every FlawlessNES challenge):
--
--   Hits  Rank      Score
--   0     Flawless  5000
--   1     A         3500
...
--   Win  = reach Stage 17. The savestate starts in Stage 16 (status bar reads
--          STAGE 16), so the win is the Stage byte ($0028) ticking above its
--          captured start value — i.e. the 16 -> 17 transition. Relative, so
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-final-bridge.State",
    ...
    -- Win = advanced to the next stage (reached Stage 17) AND still alive.

-- FlawlessNES — Castlevania Block 1 (First Level), No Damage
-- ===========================================================
-- FlawlessNES is the no-hit challenge family: clear the segment without
-- taking damage. You don't FAIL on a hit — instead every hit chips away
-- at your score. The fixed scoring table (identical for every FlawlessNES
-- challenge) is:
--
--   Hits  Rank      Score
--   0     Flawless  5000
...
-- DOES end the run — a no-hit challenge where you can infinitely respawn
-- and grind the boss isn't flawless. Survive from the savestate through
-- the Phantom Bat at the end of Block 1.
--
-- Win: the bat boss is engaged (its HP went above 0) and then drops to 0.
--      The "engaged" latch matters because this savestate starts far from
--      the boss — Boss Real Health reads 0 for the whole level, so a naive
--      "boss_hp == 0" predicate would fire on frame 1.
--
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-firstlevel.State",
    ...
    on_frame = function(state)
        ...
        -- so leftover/garbage in the boss-HP byte during normal play can't
        -- trip the win.

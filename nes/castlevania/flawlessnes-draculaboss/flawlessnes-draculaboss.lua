-- FlawlessNES — Castlevania Dracula (final boss), No Damage
-- =========================================================
-- No-hit fight against Dracula. FlawlessNES rules: you don't FAIL on a hit;
-- every hit chips your score per the fixed table (identical for every
-- FlawlessNES challenge):
--
--   Hits  Rank      Score
--   0     Flawless  5000
--   1     A         3500
...
-- DOES end the run.
--
-- Win detection — IMPORTANT: Dracula is a TWO-PHASE boss (caped first form
-- -> giant demon second form). The Boss-Health byte ($01A9) drops to 0 when
-- the FIRST form dies, before he transforms, so the usual "boss HP == 0"
-- predicate would false-win mid-fight. Instead we win on the engine's
-- System-State byte ($0018) entering the Ending state (0x0F), which only
-- happens once Dracula is fully defeated and the castle starts to crumble.
--
...
local SYSTEM_STATE = 0x0018   -- 0x05 = gameplay; 0x0F = Ending (final boss beaten)
local ENDING       = 0x0F
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-draculaboss.State",
    ...
    -- Win = the ending state — Dracula fully defeated (both phases). See the
    -- two-phase note in the header for why we don't watch boss HP here.
    win = function()
        return read_u8(SYSTEM_STATE) == ENDING
    end,

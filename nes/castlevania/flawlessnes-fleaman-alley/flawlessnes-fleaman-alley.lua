-- FlawlessNES — Castlevania Fleaman Alley (reach Stage 12), No Damage
-- ==================================================================
-- Get through the fleaman gauntlet into Stage 12 without taking a hit.
...
--   Win  = reach Stage 12. The savestate sits one stage before the target
--          (Stage 11), so the win is the Stage byte ($0028) ticking above its
--          captured start value — the 11 -> 12 transition. Relative, so it's
--          immune to whether the byte is 0- or 1-indexed, and to the exact
--          start value. Guarded by "lives unchanged" so a death-respawn can't
--          register as a win.
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-fleaman-alley.State",
    ...
    -- Win = advanced to the next stage (reached Stage 12) AND still alive.

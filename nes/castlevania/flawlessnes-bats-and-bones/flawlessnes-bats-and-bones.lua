-- FlawlessNES — Castlevania Bats & Bones (reach Stage 8), No Damage
-- ================================================================
-- Push through the bat-and-skeleton gauntlet and reach Stage 8 without
-- taking a hit. FlawlessNES rules: you don't FAIL on a hit; every hit chips
-- your score per the fixed table (identical for every FlawlessNES challenge):
--
--   Hits  Rank      Score
--   0     Flawless  5000
--   1     A         3500
...
--   Win  = reach Stage 8. The savestate starts in Stage 7 (status bar reads
--          STAGE 07), so the win is the Stage byte ($0028) ticking above its
--          captured start value — i.e. the 7 -> 8 transition. Relative, so
--          it's immune to whether the byte is 0- or 1-indexed. Guarded by
--          "lives unchanged" so a death-respawn can't register as a win.
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-bats-and-bones.State",
    ...
    -- Win = advanced to the next stage (reached Stage 8) AND still alive.
    -- The lives guard blocks a death-respawn from registering as a win.
    win = function()

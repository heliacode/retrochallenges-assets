-- FlawlessNES — Castlevania Block 3 (Third Level), No Damage
-- ==========================================================
-- Clear Block 3 and defeat its boss (the twin Mummies) without taking a hit.
-- FlawlessNES rules: you don't FAIL on a hit — every hit chips your score
-- per the fixed table (identical for every FlawlessNES challenge):
--
--   Hits  Rank      Score
--   0     Flawless  5000
--   1     A         3500
...
-- DOES end the run.
--
-- Win: the block's end boss is engaged (its HP went above 0 while it locks
--      the screen) and then drops to 0. The "engaged" latch matters because
--      this savestate starts at the level entrance, far from the boss —
--      Boss Real Health reads 0 for the traversal, so a naive "boss_hp == 0"
--      predicate would fire on frame 1. ($01A9 reaching 0 is the same win
--      the shipped mummy-boss-fight challenge uses.)
--
...
local BOSS_HEALTH  = 0x01A9   -- Boss Real Health (block boss HP)
...
-- ---------------------------------------------------------------------------
challenge.run{
    savestate           = "savestates/flawlessnes-thirdlevel.State",
    ...
    on_frame = function(state)
        ...
        -- so leftover/garbage in the boss-HP byte during traversal can't
        -- trip the win.
